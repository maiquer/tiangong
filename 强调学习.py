...
if __name__ == '__main__':
    ...
    label_csv = pd.read_csv('/content/drive/My Drive/tiangong/train_label0.csv',header = None,encoding='utf-8')
    ori_label = label_csv[1]
    label_csv[2] = label_csv[1].map(lambda x:label2num[x])
    val_ratio = 0.1
    train_csv, val_csv = train_test_split(label_csv, test_size=val_ratio, random_state=randseed, stratify=label_csv[2])
    
    class_num =6
    border_name = "same"
    l2_lr = 0.01
    train_path = '/content/drive/My Drive/tiangong/train/'
    test_path= '/content/drive/My Drive/tiangong/test/'
    workers = 0
    input_size = 256* 256
    learning_rate = 1e-3
    ...
    all_test = pd.read_csv('/content/drive/My Drive/tiangong/acc100.csv',header = None,encoding='utf-8')
    all_test[0] = all_test[0].map(lambda x:test_path+x)
    all_test[2] = all_test[1].map(lambda x:label2num[x])
    all_test = all_test.drop([1], axis=1)
...
#    # 读取测试图片列表
    test_data_list = pd.read_csv('/content/drive/My Drive/tiangong/test_img.csv',header = None,encoding='utf-8')
    test_data_list[0] = test_data_list[0].map(lambda x:test_path+x)
    ...
    all_data = TrainDataset(label_csv,
                            transform=transforms.Compose([
                                transforms.Resize((256, 256)),
                                transforms.ColorJitter(0.15, 0.15, 0.15, 0.075),
                                transforms.RandomHorizontalFlip(),
    ...
    all_val_data = TrainDataset(all_test,
                            transform=transforms.Compose([
                                transforms.Resize((256, 256)),
                                transforms.ColorJitter(0.15, 0.15, 0.15, 0.075),
                                transforms.ToTensor(),
    ...
    all_loader = DataLoader(all_data, batch_size = batch_size, shuffle=True, pin_memory=True, num_workers=workers)
    all_val_loader = DataLoader(all_val_data, batch_size=batch_size*2, shuffle=False, pin_memory=False, num_workers=workers)
    ...
    for epoch in range(1):
            train(all_loader, model, criterion, optimizer, epoch)
            # evaluate on validation set
            precision, avg_loss = validate(all_val_loader, model, criterion)
